...
export class AuthClient {
    ...
    token(name: string | null | undefined, pwd: string | null | undefined): Promise<AuthModel> {
    ...
    protected processToken(response: Response): Promise<AuthModel> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class CommonClient {
    ...
    getMenus(): Promise<MenuModel[] | null> {
        let url_ = this.baseUrl + "/api/Common/GetMenus";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetMenus(response: Response): Promise<MenuModel[] | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<MenuModel[] | null>(<any>null);
    }
}
...
export class ArticleListModel implements IArticleListModel {
    ...
    init(data?: any) {
        if (data) {
            ...
            this.updated = data["updated"] ? new Date(data["updated"].toString()) : <any>undefined;
            this.state = data["state"];
            this.file = data["file"] ? FileModel.fromJS(data["file"]) : <any>undefined;
        }
    }
    ...
    init(data?: any) {
        if (data) {
            ...
            if (data["children"] && data["children"].constructor === Array) {
                this.children = [] as any;
                for (let item of data["children"])
                    this.children!.push(MenuModel.fromJS(item));
            }
        }
    }
